...
However, not all the functionality required is in the SPA, you will have to add some additional capability, such as the facility for the bank staff to make deposits and withdrawals. 
...
Each transaction is anonymized and written to a Kafka topic. The format of the message will be provided separately.  These should be stored in a CSV file.

### Security

The security should be implemented at both the endpoints and in the service layer for enforcement of business rules.

A full list of endpoints will be provided with the security requirements for each end point. The API contract with security requirements in a supplemental document.
...
One critical requirement for the BFF is that it handes token invalidation on logout from the SPA
...
The BFF records all 401 and 403 errors ina Kafka security topic.
...
As each transaction is recorded, the corresponding fields are updated.
...
The bank accounts each have a status code which is fixed for the project. For a challenge, implement an endpoint that allows a bank employee to change the status using a PUT method.

---

On Friday June 26, the detailed specification supplements will be available. We will discuss the final spec in class and take suggestions for modifications.  

The final frozen spec will be available on Monday Jne 29
